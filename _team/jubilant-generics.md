---
title: "Jubilant Generics (P) Ltd"
date: 2022-03-01
jobtitle: "Executive Pharmacovigilance"
date_range: "March 2022 – August 2023"
location: "Noida, India"
skills: ["Argus Safety", "MedDRA", "WHODRUG", "SAE Processing", "PSURs", "PADERs", "eCTD Module 2", "EMA/MHRA Cases", "PubMed Literature Review"]
weight: 4
---

- Collected, triaged, processed, and tracked 100+ serious adverse event (SAE) reports via Argus Safety, including narrative writing and MedDRA coding
- Managed multiple case types: spontaneous, literature, and regulatory authority cases (EMA, MHRA)
- Authored Module 2 summaries — Clinical Overviews (CO), abbreviated Clinical Overviews, Nonclinical Overviews (NCO), and Clinical Expert Statements in eCTD format for global submissions
- Authored Periodic Safety Update Reports (PSURs) and Periodic Adverse Drug Experience Reports (PADERs)
- Conducted systematic PubMed literature reviews to support signal detection and identify reportable AEs/SAEs
